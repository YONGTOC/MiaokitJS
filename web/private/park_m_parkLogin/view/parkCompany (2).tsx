...
class Mien extends React.Component{
  ...
  public render() {
    console.log(this.state.mienImg.length)
    return (
      <div className={"mien"}>

        <ul>
          <ReactSwiper swiperOptions={this.swiperOptions} showPagination items={this.items}
          ...
          {this.state.mienImg.map((i, index) => {
            if (!i.pic_url) {
              ...
              return (
                <li className={this.state.mienImg.length == 1 ? "mienImgOne" : null}>
                  <img src={i.pic_url} />
                </li>
              )
            }
