...
_.each(symbol_1.IntervalSymbols, function (func, symbol) {
    // Interval 中需要用到的 symbol
    // 使用 Marker 提供的静态方法注册进去
    // 用户有可能使用动态加载 Element，所以需要注册
    g_1.Marker.symbolsFactory.register(symbol, func);
});
// 获取填充图形的图形属性
...
function _getThetaAngle(point, coord) {
    ...
    if (_.isArray(point.x)) {
        ...
        startAngle = path_1.getPointAngle(coord, startPoint);
        endAngle = path_1.getPointAngle(coord, endPoint);
...
// 描边矩形
base_1.registerShape('interval', 'hollowInterval', {
    draw: function (cfg, container) {
    ...
    getMarkerStyle: function (markerCfg) {
        ...
        shape_1.setStrokeStyle(markerStyle, markerCfg);
        return markerStyle;
    },
});
// 直线柱图
...
base_1.registerShape('interval', 'top-line', {
    draw: function (cfg, container) {
        ...
        var rectShape = container.addShape('path', {
            attrs: tslib_1.__assign(tslib_1.__assign({}, attrs), { zIndex: 0, path: rectPath }),
        });
        // 绘制 top line
        var topLinePath = [
