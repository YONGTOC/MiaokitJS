...
function getFillAttrs(cfg) {
    ...
    shape_1.setFillStyle(areaAttrs, cfg);
    return areaAttrs;
}
function getPath(cfg, smooth, shape) {
    ...
    _.each(pointsArr, function (pointsData, index) {
        ...
        if (smooth) {
            subPath = path_1.getSplinePath(points, false, cfg.constraint);
        }
        else {
            subPath = path_1.getLinePath(points, false);
        }
        if (index > 0) {
            subPath[0][0] = 'L';
        }
...
// 封闭的平滑线
base_1.registerShape('area', 'smoothLine', {
    draw: function (cfg, container) {
        var attrs = getLineAttrs(cfg);
        var path = getPath(cfg, true, this);
        return container.addShape('path', {
