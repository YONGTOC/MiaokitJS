...
var PathSegment = /** @class */ (function () {
    function PathSegment(item, preSegment, isLast) {
        this.preSegment = preSegment;
        this.isLast = isLast;
        this.init(item, preSegment);
    }
    PathSegment.prototype.init = function (item, preSegment) {
        var command = item[0];
        ...
        switch (cmd) {
            ...
            case 'A': {
                ...
                this.command = 'A';
                var params_1 = getArcParams(preEndPoint, point, fa, fs_1, rx, ry, psi);
                this.params = params_1;
                var start_1 = preSegment.subStart;
                this.subStart = start_1;
                this.endPoint = point;
                var startAngle_1 = params_1[5] % (Math.PI * 2);
                if (Util.isNumberEqual(startAngle_1, Math.PI * 2)) {
                    startAngle_1 = 0;
                }
                var endAngle = params_1[6] % (Math.PI * 2);
                ...
                this.startTangent = function () {
                    ...
                    var dx = params_1[3] * Math.cos(startAngle_1 - d_1) + params_1[1];
                    var dy = params_1[4] * Math.sin(startAngle_1 - d_1) + params_1[2];
                    return [dx - start_1.x, dy - start_1.y];
                };
                this.endTangent = function () {
                    var endAngle = params_1[6];
                    if (endAngle - Math.PI * 2 < 0.0001) {
                        endAngle = 0;
                    }
                    var dx = params_1[3] * Math.cos(startAngle_1 + endAngle + d_1) + params_1[1];
                    var dy = params_1[4] * Math.sin(startAngle_1 + endAngle - d_1) + params_1[2];
    ...
    PathSegment.prototype.isInside = function (x, y, lineWidth) {
        var self = this;
    ...
    PathSegment.prototype.draw = function (context) {
    ...
    PathSegment.prototype.getBBox = function (lineWidth) {
        ...
        switch (this.command) {
            ...
            case 'TL':
            case 'L':
                this.box = bbox_1.default.fromRange(Math.min(params[0].x, params[1].x) - halfWidth, Math.min(params[0].y, params[1].y) - halfWidth, Math.max(params[0].x, params[1].x) + halfWidth, Math.max(params[0].y, params[1].y) + halfWidth);
                break;
            case 'SQ':
            case 'Q':
                ...
                this.box = bbox_1.default.fromRange(Math.min.apply(Math, xDims) - halfWidth, Math.min.apply(Math, yDims) - halfWidth, Math.max.apply(Math, xDims) + halfWidth, Math.max.apply(Math, yDims) + halfWidth);
                break;
            case 'C':
            ...
            case 'A': {
                ...
                this.box = bbox_1.default.fromRange(minX - halfWidth, minY - halfWidth, maxX + halfWidth, maxY + halfWidth);
                break;
            }
        }
    };
...
//# sourceMappingURL=path-segment.js.map