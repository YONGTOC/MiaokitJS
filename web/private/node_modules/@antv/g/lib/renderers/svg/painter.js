...
var marker_1 = require("../../shapes/marker");
var symbolsFactory = marker_1.default.symbolsFactory;
var SHAPE_TO_TAGS = {
    rect: 'path',
    circle: 'circle',
    line: 'line',
...
var Painter = /** @class */ (function () {
    function Painter(dom) {
        ...
        this.context = new defs_1.default(canvasDom);
        this.toDraw = false;
        return this;
    }
    ...
    Painter.prototype._drawGroup = function (model, index) {
        var cfg = model.cfg;
    ...
    Painter.prototype._drawChildren = function (parent) {
        var self = this;
    ...
    Painter.prototype._drawShape = function (model, index) {
        var self = this;
    ...
    Painter.prototype._updateShape = function (model) {
        var self = this;
        var attrs = model.attrs;
        var formerAttrs = model.cfg.attrs;
        if (!formerAttrs) {
            return;
        }
        if (!model.cfg.el) {
    ...
    Painter.prototype._setAttribute = function (model, name, value) {
        var type = model.type;
        var attrs = model.attrs;
        var el = model.cfg.el;
        ...
        if (type === 'polygon' && name === 'points') {
            ...
            if (Util.isArray(value)) {
                value = value.map(function (point) { return point[0] + ',' + point[1]; });
                value = value.join(' ');
            }
    ...
    Painter.prototype._createDom = function (model, index) {
        var type = SHAPE_TO_TAGS[model.type];
        var attrs = model.attrs;
        var parent = model.cfg.parent;
        if (!type) {
        ...
        model.cfg.el = shape;
        if (parent) {
            var parentNode = parent.cfg.el;
            if (typeof index === 'undefined') {
                parentNode.appendChild(shape);
            }
            else {
                var childNodes = parent.cfg.el.childNodes;
    ...
    Painter.prototype._assembleMarker = function (marker, attrs) {
        var d = marker._getPath();
        if (Util.isArray(d)) {
            return d
                .map(function (path) {
                return path.join(' ');
    ...
    Painter.prototype._assembleRect = function (attrs) {
        var x = attrs.x;
        var y = attrs.y;
        var w = attrs.width;
        var h = attrs.height;
    ...
    Painter.prototype._formatPath = function (value) {
        value = value
            .map(function (path) {
            return path.join(' ');
        })
    ...
    Painter.prototype._setTransform = function (model) {
        var matrix = model.attrs.matrix;
        var el = model.cfg.el;
        var transform = [];
        for (var i = 0; i < 9; i += 3) {
    ...
    Painter.prototype._setImage = function (model, img) {
        var attrs = model.attrs;
        var el = model.cfg.el;
    ...
    Painter.prototype._updateFan = function (model) {
    ...
    Painter.prototype._updateText = function (model) {
        var self = this;
        var attrs = model.attrs;
        var formerAttrs = model.cfg.attrs;
        var el = model.cfg.el;
    ...
    Painter.prototype._setFont = function (model) {
        var el = model.get('el');
        var attrs = model.attrs;
    ...
    Painter.prototype._setText = function (model, text) {
        var el = model.cfg.el;
        var baseline = model.attrs.textBaseline || 'bottom';
        if (!text) {
            el.innerHTML = '';
        }
        else if (~text.indexOf('\n')) {
            var x_1 = model.attrs.x;
            ...
            Util.each(textArr, function (segment, i) {
                if (i === 0) {
                    if (baseline === 'alphabetic') {
                        arr_1 += "<tspan x=\"" + x_1 + "\" dy=\"" + -textLen_1 + "em\">" + segment + "</tspan>";
                    }
                    else if (baseline === 'top') {
                        arr_1 += "<tspan x=\"" + x_1 + "\" dy=\"0.9em\">" + segment + "</tspan>";
                    }
                    else if (baseline === 'middle') {
                        arr_1 += "<tspan x=\"" + x_1 + "\" dy=\"" + -(textLen_1 - 1) / 2 + "em\">" + segment + "</tspan>";
                    }
                    else if (baseline === 'bottom') {
                        arr_1 += "<tspan x=\"" + x_1 + "\" dy=\"-" + (textLen_1 + LETTER_SPACING) + "em\">" + segment + "</tspan>";
                    }
                    else if (baseline === 'hanging') {
                        arr_1 += "<tspan x=\"" + x_1 + "\" dy=\"" + (-(textLen_1 - 1) - LETTER_SPACING) + "em\">" + segment + "</tspan>";
                    }
                }
                else {
                    arr_1 += "<tspan x=\"" + x_1 + "\" dy=\"1em\">" + segment + "</tspan>";
                }
            });
            el.innerHTML = arr_1;
        }
    ...
    Painter.prototype._setClip = function (model, value) {
        var el = model.cfg.el;
        if (!value) {
    ...
    Painter.prototype._setColor = function (model, name, value) {
        var el = model.cfg.el;
        var defs = this.context;
        if (!value) {
        ...
        if (/^[r,R,L,l]{1}[\s]*\(/.test(value)) {
            ...
            var id = defs.find('pattern', value);
            if (!id) {
    ...
    Painter.prototype._setShadow = function (model) {
        var el = model.cfg.el;
        var attrs = model.attrs;
        var cfg = {
